# Contributing to segb-forensic

Thanks for your interest in improving `segb-forensic`. These crates parse
untrusted Apple SEGB (Biome) streams acquired from potentially compromised
systems, so correctness and robustness are not negotiable. The bar is high and
the workflow is strict — please read this before opening a pull request.
...
Apple Biome streams and an independent oracle (`ccl-segb`), not only synthetic
fixtures.
...
- **Fuzzing** — changes to a parser must keep its fuzz target green. Run the
  ...
  cargo +nightly fuzz run segb2       # or: segb1, proto, menuitem, forensic
  ```
...
- Fail loud — surface malformed input as a typed `SegbError` with enough context
